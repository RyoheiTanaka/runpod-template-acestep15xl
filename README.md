...
as a Pod to use the ComfyUI web UI, or as a Serverless endpoint to run
workflows through the queue API. The same image serves both: ComfyUI keeps
...
| Serverless endpoint | Submit workflows through the queue API, with autoscaling. |
